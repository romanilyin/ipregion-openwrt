...
IPRegion это CLI и LuCI-приложение для OpenWrt 25.12.4+, которое проверяет, как GeoIP API, популярные сервисы, CDN endpoint-ы и AI-провайдеры видят маршрут роутера, интерфейс или SOCKS5-прокси.
...
Release APK пакеты собираются как `noarch`, поэтому одни и те же assets предназначены для поддерживаемых OpenWrt 25.12.4+ targets на разных CPU-архитектурах.
