...
Проверенная runtime-цель: OpenWrt 25.12.1+. Подготовка OpenWrt 24.10.* экспериментальная в отдельной 24.10 branch и должна пройти проверку на реальном железе до public release.
...
Release APK пакеты собираются как `noarch`, поэтому одни и те же assets предназначены для поддерживаемых OpenWrt targets на разных CPU-архитектурах. Public releases остаются проверенными для OpenWrt 25.12.1+, пока не пройдена router smoke проверка 24.10.
